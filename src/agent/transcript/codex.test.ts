...
describe("Codex adapter", () => {
  it("emits working (no text) for task_started", () => {
    const line = { type: "event_msg", payload: { type: "task_started" } }
  ...
  it("emits completed with cached agent_message fallback", () => {
    ...
    adapter.processLine({
      type: "event_msg",
      payload: { type: "agent_message", message: "Final result" },
    })
  ...
  it("agent_message does not emit text in working events", () => {
    const result = processLine({
      type: "event_msg",
      payload: { type: "agent_message", message: "Mid-task" },
    })
    expect(result!.type).toBe("working")
    expect(result!.text).toBeUndefined()
  })
  ...
  it("ignores non-event_msg lines", () => {
    ...
    expect(processLine({ event_msg: { payload: { type: "task_complete" } } })).toBeUndefined()
  })

  it("ignores intermediate response_item events", () => {
    const line = { type: "event_msg", payload: { type: "response_item" } }
