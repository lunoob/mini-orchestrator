...
import { createSessionApiServer } from "@src/session/server"
import { waitForTurn } from "@src/session/turn-wait"
import type { SessionClient } from "@src/session/client"
import type { SessionItem, SessionRecord, SessionStreamEvent } from "@src/session/types"

const clientModulePath = "@src/session/client"
...
describe("SessionClient", () => {
  test("creates a session and sends an idempotent prompt through the Session API", async () => {
    const { createSessionClient } = await import(clientModulePath) as typeof import("@src/session/client")
