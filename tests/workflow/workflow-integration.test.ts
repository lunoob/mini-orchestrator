...
describe("resume session reconnection", () => {
  ...
  it("reconnects to alive ready session via startRuntimeAgents", async () => {
    vi.doMock("@src/workflow/agent-runtime", async (importOriginal) => {
      const original = await importOriginal<typeof import("@src/workflow/agent-runtime")>()
      return { ...original }
    })

    const { startRuntimeAgents, stopRuntimeAgents } = await import("@src/workflow/agent-runtime")
    const client = makeFakeClient()
    const runtime = makeBaseRuntime(client)
